...
namespace InvoiceManager.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        private EditInvoiceViewModel PrepareInvoiceVm(Invoice invoice, string userId)
        {
            ...
            {
                Invoice = invoice,
                Heading = invoice.Id == 0 ? "Dodawanie nowej faktur " : "Faktura",
        ...
        public ActionResult InvoicePosition(int invoiceId, int invoicePositionId = 0) 
        {
            ...
            var invoicePostion = invoicePositionId == 0 ? GetNewPosition(invoiceId, invoicePositionId) : _invoiceRepository.GetInvoicePosition(invoicePositionId, userId);

            var vm = PrepareInvoicePositionVm(invoicePostion);

            return View(vm); 
        }

        private EditInvoicePositionViewModel PrepareInvoicePositionVm(InvoicePosition invoicePostion)
        {
            return new EditInvoicePositionViewModel
            {
                InvoicePosition = invoicePostion,
                Heading = invoicePostion.Id == 0 ? "Dodawanie nowej pozycji" : "Edycja pozycji",
        ...
        public ActionResult DeleteInvoice(int id)
        {
            ...
            {
                //Logowanie błędów do pliku(opcja)
